...
SCHEMA_VERSION = 1
TENSOR_SCHEMA = 'fine-row-major-256x64_coarse-row-major-64x64_memory128_left-stay-right-v1'
...
def read_checkpoint(path, *, dependencies=None, dataset_fingerprint=None, stage=None):
    ...
    if checkpoint.get('tensor_schema') != TENSOR_SCHEMA:
...
def _construct():
    ...
    return {'E': Encoder, 'D': Decoder, 'H': PositionReadout,
...
def export_bundle(perception, memory, predictor, output):
    ...
    models = {k: m.state_dict() for k, m in system.items() if k != 'statistics'}
