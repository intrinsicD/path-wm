# Adapted from https://github.com/galilai-group/stable-worldmodel@6f1e499e9cc0c898d326112f485c1062c3d20f24:stable_worldmodel/spaces.py (MIT)
...
class Dict(spaces.Dict):
    ...
    def update(self, keys: Iterable[str]) -> None:
        ...
        if len(keys_set) == 1 and 'all' in keys_set:
            ...
            for v in filter(keys_set.__contains__, order):
                try:
                    var_path = v.split('.')
                    nested.get_in(self, var_path).sample()
    ...
    def set_init_value(self, variations_values: dict) -> None:
        ...
        for k, v in variations_values.items():
            try:
                var_path = k.split('.')
                space = nested.get_in(self, var_path)
