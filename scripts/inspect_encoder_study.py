...
from world_model.curriculum.pose_accessibility import setup
...
def main():
    ...
    for arm,(depth,exchange) in ARMS.items():
        ...
        fig,axes=plt.subplots(5,6,figsize=(11,9),layout='constrained')
        for c in range(6):
            ...
            axes[1,c].imshow(fc[c]);axes[2,c].imshow(cc[c])
            for row,name,size in [(3,'fine',16),(4,'coarse',8)]:
            ...
            for row in range(5):axes[row,c].set_xticks([]);axes[row,c].set_yticks([])
        for row,label in enumerate(['Pose (+ truth, × pred)','Fine features · PCA','Coarse features · PCA','Fine attention entropy','Coarse attention entropy']):axes[row,0].set_ylabel(label,fontsize=9)
