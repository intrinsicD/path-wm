...
| PushT geometry | Independent fused width-128 spatial trunk with two residual blocks; two location heatmaps and expected coordinates; a separate learned orientation pooling/readout with 128-unit MLP → two values | Existing normalized pose MSE initially; predict pusher XY, simulator body-origin XY, cos θ and sin θ. Orientation is not obtained by subtracting the location predictions |
...
Direct decoder access to memory is useful only for an output that needs history or a belief about hidden state. Compare visual-only, memory-only and combined input with suitable trained controls. Standard image reconstruction should target visible pixels; hidden-object completion is a different supervised or probabilistic task. Train a future decoder on its actual predicted-state distribution when it will consume predictions.
