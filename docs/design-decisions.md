...
18. *Action sensitivity is necessary, not sufficient.* Opposite-action separation can increase even when the predictor's response is unrelated to the true transition. Development diagnostics must pair s(w) with identity, zero-action, and shuffled-action transition errors; a candidate action anchor succeeds only when the correct action improves over those controls. These are diagnostics, not post-hoc E1 thresholds.
