...
| Session memory → §5 | Discussed | Discussed recent inputs, hierarchical history, external entity storage, retrieval, latent versus raw evidence and provenance. On 15 September, reviewed the user-proposed multimodal World State: stable IDs, learned components/relations, evidence, reversible attribution and optional spatial representations. See docs/world-state-proposal-review.md. | Specify candidate/learning/commit contracts and bounded identity/state/history/restart experiments before a unified store. Evidence starts with M1; concepts follow separately. Current storage validation does not cover this proposed extension. |
| Task workspace → §6 | Discussed | Discussed focus on entities and relationships, recalled context, internal thinking and interleaving internal/external actions. Reviewed separate binding-candidate and task retrieval, with bounded evidence/context and an explicit unresolved result. | Define query learning and retrieval budgets in the training-time interface; compare oracle, flat episodic, structured and no-history conditions. General learned retrieval and stopping remain open. |
