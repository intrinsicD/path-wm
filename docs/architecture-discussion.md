...
| Modality encoders → §2–3 | Discussed | Discussed modality stems, residual transformer blocks per scale, hierarchy merges, fusion, temporal handling and reconstruction probes. On 14 September, the user asked whether architectural structure and a small parameter count can reduce visual training-data requirements. On 15 September, the user specified a spatial VAE with separate lossless rearrangement, residual processing and channel projection; Claude review was reconciled. | The proposed spatial VAE is specified in docs/spatial-vae-design.md; implementation, rate/distortion quality and sample efficiency remain unvalidated. State/recall repair and data/capacity comparisons remain separate. |
...
| Modality outputs → §7 | Discussed | Discussed every encoder/decoder pair, state-conditioned outputs, image detail, conditional generation and replacement interfaces. On 15 September, discussed a mirrored PixelShuffle VAE decoder, Gaussian loss/KL units and a future state-conditioned spatial-latent producer. | The proposed VAE decoder has not been implemented or trained. Reconstruction, prior sampling and controlled generation need distinct tests; existing image-output failures remain open. |
