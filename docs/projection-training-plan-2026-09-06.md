...
## Reporting repair after outcome 22 (2026-09-07)

The seed3073 PushT 750-update calibrated evaluation completed (1/50), but canonical static-chart extraction exceeded its unchanged 4 MB browser-output limit. The coordinator and dependent finalizer stopped; raw results and the last verified HTML remain preserved. Repair only repeated identity text: exact metric/context rows will reference a stable short record key, with a complete one-to-one key/run/source mapping in the inventory. Every exact value and full source name remains recoverable; scientific inputs, cases, budgets, renderer probes and size limits stay unchanged. Add an informative row-conservation/join regression before implementation, verify the actual dashboard and a final-size synthetic fixture, then resume using the existing completed-raw recovery path without rerunning control.
