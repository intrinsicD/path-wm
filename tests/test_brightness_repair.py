...
def test_augmented_cache_live_alignment_and_frozen_gradient_contract():
    ...
    d = MemoryOutputEpisodes(16, seed=66, curriculum="relocation").with_input_offsets(
        [-16, 0, 16]
    )
...
def test_median_centering_is_per_frame_preserves_metadata_and_rejects_clipping():
    ...
    class Capture(torch.nn.Module):
        ...
        def forward(self, obs, **kw):
            return obs

    m = PixelMedianCentering(Capture(), [40 / 255] * 3)
    x = MemoryOutputEpisodes(16, seed=67, curriculum="relocation").batch(range(2))[
        "images"
    ]
    ...
    class Provenance:
        def derived(self):
            return "derived-memory-evidence"

    a = m(Observation(x, times, valid, Provenance()))
    assert a.provenance == "derived-memory-evidence"
    before = x.clone()
    ...
    assert (
        torch.equal(a.times, times)
        and torch.equal(a.valid, valid)
        and torch.equal(x, before)
    )
    invalid = x.clone()
    invalid[0, 1] = float("nan")
    mask = valid.clone()
    mask[0, 1] = False
