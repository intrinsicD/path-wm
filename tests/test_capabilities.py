...
def test_visual_perturbations_keep_paired_controls_and_reflection_labels():
    ...
    for condition in ('original', 'mirror', 'swap_red_blue', 'grayscale', 'dim', 'noise'):
        changed = transform_visual(data, condition, seed=106401)
        np.testing.assert_array_equal(changed.images[::2, -2:], changed.images[1::2, -2:])
        np.testing.assert_array_equal(changed.labels, data.labels ^ (condition == 'mirror'))
...
def test_scores_and_comparison_never_promote_unmeasured_or_changed_cases():
    ...
    with pytest.raises(ValueError):
        score(float('nan'), 0.9)
    before = dict(protocol_sha256='a', cases=[dict(id='recall', input_sha256='b',
                  checkpoint='old', metrics=dict(accuracy=0.5))])
    ...
    for broken in ('protocol_sha256', 'input_sha256'):
        ...
        (candidate if broken == 'protocol_sha256' else candidate['cases'][0])[broken] = 'changed'
        with pytest.raises(ValueError):
