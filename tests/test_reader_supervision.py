...
def test_auxiliary_reader_changes_only_training_gradients_and_stays_frozen():
    ...
    unassisted, _ = objective(model, batch, True, repair=True)
    original = model.observe_history(batch["images"])["final"].memory.values.clone()
    model.workspace_reference = TokenProbe(
        model.working(model.observe_history(batch["images"])["stored"])
...
def test_reference_requires_valid_weight_and_matching_finite_export(tmp_path):
    ...
    export = dict(
        width=16,
        stages=["stored_working"],
        model={"stored_working." + k: v for k, v in probe.state_dict().items()},
    )
