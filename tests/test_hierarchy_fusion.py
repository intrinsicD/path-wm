...
def test_handwritten_training_freezes_exact_modules_and_preserves_gradient_path(
    tmp_path, part
):
    ...
    info = configure_training(
        model, initial_weights=path, train_part=part, loss_mode="rgb"
    )
    ...
    for name, module in modules.items():
        expected_change = name != "mask" and (
            part == "both"
            or (part == "encoder" and name == "encoder")
            or (part == "decoder" and name == "rgb")
        )
...
def test_opening_dormant_decoder_branch_keeps_output_and_enables_weight_gradient(
    tmp_path,
):
    from experiments.hierarchy_fusion import configure_training, rgb_objective
...
def test_handwritten_frozen_training_resumes_exactly(tmp_path, decoder_rate):
    from experiments.hierarchy_fusion import (
        configure_training,
        rgb_objective,
    ...
    data = Frames(
        ...
        range(4),
        labels={
            "mask": np.zeros((4, 1, 64, 64), dtype="float32"),
            "valid": np.ones((4, 1, 64, 64), dtype="float32"),
        },
    )
    ...
    for k, v in complete["model"].items():
        assert torch.equal(v, resumed["model"][k])
