...
def test_all_modalities_outputs_and_gradients():
    ...
    obs = {
        ...
        'video': Observation(torch.rand(2, 2, 3, 16, 16), torch.tensor([[-1., 0.]]).expand(2, -1)),
        'audio': Observation(torch.randn(2, 2, 32), torch.tensor([[-1., 0.]]).expand(2, -1)),
    ...
    loss = sum(x.square().mean() for x in output.values()) + future.tokens.square().mean()
    loss.backward()
    ...
    assert all(not x.requires_grad for x in trace.values() if isinstance(x, torch.Tensor))
...
def test_masks_hide_values_and_future_observations_are_rejected():
    ...
    with pytest.raises(ValueError, match='valid observation'):
        model.observe(state, {'video': Observation(values, times, valid & False)}, time=0)
    with pytest.raises(ValueError, match='backward'):
        model.observe(replace(state, time=state.time + 1), {'image': image_observation()}, time=0)


def test_thinking_memory_imagination_and_state_roundtrip():
    model = small().eval()
    observed = model.observe(model.initial_state(2), {'image': image_observation()}, time=0)
    ...
    assert not torch.equal(thought.tokens[:, model.layout['reasoning']], before[:, model.layout['reasoning']])
    ...
    torch.manual_seed(3)
    equal_tree(sampled.to_dict(), model.imagine(thought, None, dt=1, sample=True).to_dict())


def test_text_decoder_is_causal_and_new_input_needs_only_an_adapter():
    model = small().eval()
    model.encoders['sensor'] = VectorEncoder(5, 16)
    state = model.observe(model.initial_state(2), {
    ...
    changed = model.intervene(state, 'entities', 0.)
...
def test_planner_matches_known_transition_and_preserves_caller():
    ...
    class Toy(torch.nn.Module):
        ...
        def imagine(self, state, action, dt=1., **kwargs):
            return replace(state, tokens=state.tokens + action[:, :1, None] * dt,
                           time=state.time + dt, imagined=True)

    candidates = torch.tensor([[[[0., 0.]], [[1., 0.]], [[-1., 0.]]]]).expand(2, -1, -1, -1)
    target = state.tokens + 1
    original = state.tokens.clone()
    result = plan(Toy(), state, candidates, lambda s: (s.tokens - target).square().mean((1, 2)),
                  lower=-1, upper=1)
    assert result.indices.tolist() == [1, 1]
    assert result.scores[:, 1].tolist() == [0., 0.]
    torch.testing.assert_close(state.tokens, original, rtol=0, atol=0)
    with pytest.raises(ValueError, match='bounds'):
        plan(Toy(), state, candidates * 2, lambda s: s.tokens.mean((1, 2)), lower=-1, upper=1)
    with pytest.raises(ValueError, match='finite'):
        plan(Toy(), state, candidates, lambda s: s.time * float('nan'), lower=-1, upper=1)
...
def test_learning_proposal_accepts_improvement_and_rolls_back_regression():
    ...
    accepted = try_improvement(model, optimizer, proposal, evaluate, primary='error',
                               min_improvement=0.01, tolerances={'error': 0.})
    ...
    rejected = try_improvement(model, optimizer, bad, evaluate, primary='error',
                               min_improvement=0.01, tolerances={'error': 0.})
