...
def population():
    ...
    logits = torch.cat(
        [
            torch.nn.functional.one_hot(b["labels"][:, i], n)
...
def test_scene_scores_keep_duplicate_blocks_and_expose_shape_failure():
    ...
    for i, b in enumerate(result["blocks"]):
        ...
        assert (
            s["factual_accuracy"]
            == s["image_color_accuracy"]
            == s["image_side_accuracy"]
            == 1
        )
...
@pytest.mark.parametrize(
    "corruption", ["length", "labels", "targets", "metadata", "block_order"]
)
...
def test_scene_report_is_training_only_and_escapes_labels(tmp_path):
    ...
    assert "Training fit by scene" not in render_report(tmp_path).read_text()
    assert (
        training_scene_scores(MemoryOutputEpisodes(16, curriculum="relocation"), {})
        is None
    )


def test_target_alignment_allows_only_normalization_roundoff():
    from experiments.memory_output import training_scene_scores

    d, a = population()
    a["target"] = np.nextafter(a["target"], np.float32(1))
    assert len(training_scene_scores(d, a)["blocks"]) == 3
    a["target"][0, 0, 0, 0] += 1 / 255
