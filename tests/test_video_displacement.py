...
def test_displacement_spec_rejects_ambiguous_or_colliding_groups(tmp_path):
    ...
    spec = dict(
        ...
        train=[2, 4, 6, 8],
        evaluation=dict(
            known=[2, 4], wide=[6, 8], intermediate=[3, 5, 7], extrapolation=[10, 11]
        ),
    )
    ...
    for name in ("train", "validation", "confirm_x", "../x"):
        path.write_text(
            json.dumps(dict(spec, evaluation=dict(spec["evaluation"], **{name: [1]})))
        )
        with pytest.raises(ValueError):
            load_displacements(path)
...
def test_phase_sampler_matches_content_and_resume_across_support_sizes():
    ...
    for _ in range(20):
        indices = [
            sample_phase_pairs(
                lambda n, k: torch.randint(n, (k,), generator=rng), groups, 8
            )
            for rng, groups in zip(rngs, (ga, gb))
        ]
    ...
    expected = sample_phase_pairs(
        lambda n, k: torch.randint(n, (k,), generator=rngs[0]), ga, 8
    )
    rngs[1].set_state(state)
    actual = sample_phase_pairs(
        lambda n, k: torch.randint(n, (k,), generator=rngs[1]), ga, 8
    )
    assert torch.equal(expected, actual)
    broken = b["metadata"].clone()
    broken[0, 1] = 3
    with pytest.raises(ValueError):
