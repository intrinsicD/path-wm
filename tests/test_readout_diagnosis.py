...
def test_stage_capture_preserves_forward_gradient_and_rng():
    ...
    torch.testing.assert_close(stages['posterior'].reshape(2,4,8).sum(-1), torch.ones(2,4))
...
def test_probe_recovers_facts_and_uses_training_statistics():
    from pathwm.evaluation.modality_readout import fit_factor_probe, predict_factor_probe

    rng = np.random.default_rng(12)
    labels = np.array([[a,b,c] for a in range(3) for b in range(3) for c in range(2)])
    facts = np.concatenate([np.eye(n)[labels[:,i]] for i,n in enumerate((3,3,2))], 1)
