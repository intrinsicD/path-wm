...
def test_photo_split_selection_keeps_groups_disjoint(tmp_path):
    ...
    manifest=dict(schema="curriculum-coco-v1",frames_sha256=file_hash(tmp_path/"frames.npy"),
                  transform="fixture",records=[dict(group=i,file=str(i),canonical_sha256=str(i)) for i in range(12)],
                  splits=dict(train=list(range(6)),validation=[6,7,8],test=[9,10,11]))
    (tmp_path/"manifest.json").write_text(json.dumps(manifest))
    a=photo_data(tmp_path,(4,2,2),13);b=photo_data(tmp_path,(4,2,2),13)
...
def test_photo_history_hides_current_target_and_swaps_metrics():
    ...
    x=torch.stack([torch.full((3,64,64),.2),torch.full((3,64,64),.8)])
    ...
    arrays=dict(target=x,teacher=x,mean=torch.full_like(x,.5),ordinary=x,reset=x,
                erased=torch.full_like(x,.5),swapped=x.flip(0))
    metrics=score(arrays)
    assert metrics['reset']['rgb_mse']==0
    assert metrics['swapped_target']['rgb_mse']==0
    assert metrics['swapped_original']['rgb_mse']>.3
...
def test_real_photo_continuation_freeze_resume_and_export(tmp_path):
    ...
    config['feature_generator']=dict(objective='flow',depth=1,fusion_depth=1,steps=2,hidden_width=16)
    configure_generator(model,config['feature_generator'])
    source=tmp_path/'source.pt';torch.save(dict(model=model.state_dict(),settings=config),source)
    ...
    cfg=dict(config,**settings());cfg.update(steps=4,batch_size=2,disk_free_gib=0,wall_seconds=120)
    def fit(name,resume=False,stop_after=None):
        ...
        train(m,cache,data,validation,output=tmp_path/name,config=c,resume=resume,stop_after=stop_after)
    ...
    full,a=fit('full');fit('resume',stop_after=1);_,b=fit('resume',resume=True)
    for key in ['model','optimizer','torch','sampler','step']:equal_tree(a[key],b[key])
    ...
    loaded.agent.encoders['image'].forward=lambda *_:pytest.fail('image encoder called')
