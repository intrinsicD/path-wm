...
def test_corpus_balances_each_prefix_length_and_holds_out_templates():
    ...
    assert [len(families[s]) for s in ("calibration", "validation", "test")] == [
        6,
        2,
        4,
    ]
    ...
    for family in {r["family"] for r in stress}:
        ...
        assert len(pair) == 2 and Counter(pair[0]["question"].encode()) == Counter(
            pair[1]["question"].encode()
        )
...
def test_probe_test_labels_cannot_affect_fitted_weights_or_predictions():
    ...
    changed = [
        r | dict(label=1 - r["label"]) if r["split"] == "test" else r for r in rows
    ]
    other, weights = request_probe(x, changed)
    assert np.array_equal(result["predictions"], other["predictions"])
    for key in fitted:
        torch.testing.assert_close(fitted[key], weights[key], rtol=0, atol=0)
...
def test_captured_request_stages_are_actual_path_and_preserve_physical_state():
    ...
    assert recipe.state_hash(model) == weights and torch.equal(
        rng, torch.get_rng_state()
    )
    assert torch.equal(before.tokens, state.tokens) and torch.equal(
        before.logits, state.logits
    )
...
def test_interpreter_only_fit_keeps_request_free_outputs_and_all_other_state_exact():
    ...
    assert all(
        n.startswith("core.agent.task_interpreter.")
    ...
    rows = recipe.grounded_records(
        data, "VID.order", request_contrasts=True, request_profile="balanced"
    )
    ...
    loss = recipe.grounded_objective(
        model,
        model.core(inputs, requests=[rows[i]["question"] for i in (0, 1)]),
        targets,
    )
    loss.backward()
    assert any(
    ...
    for n, v in model.state_dict().items():
        if not n.startswith("core.agent.task_interpreter."):
            assert torch.equal(v, before[n])
    with torch.no_grad():
        torch.manual_seed(32)
