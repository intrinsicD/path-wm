...
@pytest.mark.parametrize("kwargs", [
    dict(frame_offsets=(-12, 12, -8)), dict(rgb_offset=(8, -8, 4)),
    dict(background_offset=(12, 0, -8)), dict(texture=8),
    dict(background_offset=(48, 48, 48)), dict(radius=12),
    dict(clutter=True), dict(gain=0.75), dict(shadow=12),
])
def test_scene_preserves_counterfactuals_and_semantic_targets(kwargs):
    ...
    assert result.identity["images_sha256"] == hashlib.sha256(result.images.tobytes()).hexdigest()
...
def test_background_changes_do_not_recolor_objects_or_selection_cue():
    ...
    for kw in [dict(background_offset=(12, 0, -8)), dict(texture=8), dict(clutter=True)]:
...
@pytest.mark.parametrize("kwargs", [dict(radius=13), dict(radius=True), dict(gain=float('nan')),
    dict(frame_offsets=(0, 0)), dict(rgb_offset=(True, 0, 0)), dict(texture=-1),
    dict(clutter=1), dict(shadow=-1), dict(gain=2), dict(background_offset=(-100, 0, 0))])
...
def test_scene_cannot_retransform_or_use_incompatible_curriculum():
    for d in [base().with_scene(texture=8), base().with_input_offsets([0]),
...
def test_frame_channel_cancellation_and_discarded_intensity():
    ...
    b = d.with_scene(frame_offsets=(-12, 12, -8), rgb_offset=(4, -4, 2)).batch(range(4))["images"]
    times = torch.arange(3).float().expand(4, -1)
    m = PixelMedianCentering(Capture(), [40/255]*3)
    torch.testing.assert_close(m(Observation(a, times)).values, m(Observation(b, times)).values, atol=1e-7, rtol=0)
    # If absolute lightness were a target, this representation cannot distinguish it.
    x = torch.full_like(a, 0.25)
    torch.testing.assert_close(m(Observation(x, times)).values, m(Observation(x+0.25, times)).values)
...
def test_scene_cli_is_evaluation_only(monkeypatch, tmp_path):
    ...
    seen = []
    monkeypatch.setattr(recipe, "evaluate_export", lambda weights, data, **kw: seen.append(data))
    args = ["memory_output", "--weights", str(weights), "--output", str(tmp_path/'out'),
            "--test-seed", "93", "--scene", "temporal-offset"]
    monkeypatch.setattr(sys, "argv", args + ["--evaluate-only"])
    recipe.main()
    assert seen[0].identity["input_transform"]["parameters"]["frame_offsets"] == [-12, 12, -8]
