...
def config():
    ...
    return dict(settings(), dataset="entities", width=16, history=3, horizon=1,
                train_windows=64, validation_windows=32, improve_every=0,
                batch_size=8, evaluate_every=1, max_seconds=450., seed=31)
...
def test_entity_contract_oracle_pairs_and_split():
    ...
    for data in sets:
        ...
        assert data.final_view_bounds() == dict(identity=.5, state=.25, effect=.25)
...
def test_entity_resume_cache_and_final_only(tmp_path, monkeypatch):
    ...
    states = [torch.load(tmp_path/n/"last.pt", weights_only=True) for n in ("resume", "full")]
    for state in states:
