...
def state_fixture():
    ...
    return LatentState(tokens, torch.zeros_like(tokens), torch.full((2,), 3.),
                       torch.full((2,), 3.), observation_count=3,
                       memory=MemoryBank(keys, values, torch.tensor([[0., 1., 2.], [0., 1., 2.]]),
                                         ('first', 'second', 'third')))
...
def test_time_tags_follow_selected_values_and_preserve_storage():
    ...
    torch.testing.assert_close(timed.read(state), expected.flatten(1, 2), atol=0, rtol=0)
    assert torch.equal(state.memory.values, raw)
    assert not list(timed.parameters())
    zero = replace(state, memory=replace(state.memory, times=torch.full((2, 3), 3.)))
...
def test_time_read_is_clock_invariant_and_storage_permutation_equivariant():
    ...
    shifted = replace(state, time=state.time+16, observed_time=state.observed_time+16,
                      memory=replace(state.memory, times=state.memory.times+16))
    assert torch.equal(memory.read(shifted), expected)
    p = [2, 0, 1]
    bank = state.memory
    reordered = replace(state, memory=replace(bank, keys=bank.keys[:, p], values=bank.values[:, p],
                                             times=bank.times[:, p], sources=tuple(bank.sources[i] for i in p)))
    assert torch.equal(memory.read(reordered), expected)
    assert not torch.equal(memory.read(replace(state, memory=replace(bank, times=bank.times.flip(1)))), expected)
...
def test_timestamp_interventions_leave_untimed_query_unchanged():
    ...
    images = MemoryOutputEpisodes(16, seed=64, curriculum='relocation').batch(range(4))['images']
    with torch.no_grad():
        ...
        for k, v in bank.items():
            assert torch.equal(getattr(history.memory, k), v) if isinstance(v, torch.Tensor) else getattr(history.memory, k)==v
