...
def test_full_prevalidation_and_partial_failure_with_exact_pending_outputs():
    ...
    calls = []
    handle = model.decoders["image"].register_forward_hook(
        lambda *args: calls.append(1)
    )
    with pytest.raises(ValueError, match="disabled"):
        model.emit(
            state, session, (request("image", USER), request("text")), produced_by=AGENT
        )
    assert not calls
    ...
    result = model.emit(
        state,
        session,
        (request("image", USER), request("audio", USER)),
        produced_by=AGENT,
    )
    handle.remove()
    ...
    candidate = model.emit(
        state, session, (request("image", USER, "candidate"),), produced_by=AGENT
    )
...
def test_video_internal_image_dependency_and_new_output_adapter():
    ...
    state = observed(model)
    session = task(
        OutputControl("image", "disabled", USER),
        OutputControl("video", "required", USER),
    )
...
def test_gates_hold_for_arbitrary_logits_and_finish_requires_fulfillment():
    ...
    for _ in range(32):
        ...
        assert (
            selection.raw_operation == OPERATIONS[int(raw.operation_logits.argmax(-1))]
        )
...
def test_loopback_preserves_world_state_and_cannot_be_laundered_into_memory():
    ...
    session = task()
    output = model.emit(state, session, (request("image"),), produced_by=AGENT).outputs[
        0
    ]
    ...
    for name in ("time", "observed_time", "log_scale"):
        torch.testing.assert_close(
            getattr(state, name), getattr(reflected, name), rtol=0, atol=0
        )
    assert (
        state.imagined == reflected.imagined
        and state.observation_count == reflected.observation_count
    )
    for role in ("sensory", "entities", "context"):
        torch.testing.assert_close(
            state.tokens[:, model.layout[role]],
            reflected.tokens[:, model.layout[role]],
            rtol=0,
            atol=0,
        )
    ...
    updated = model.observe(
        reflected,
        {"image": Observation(torch.rand(1, 1, 3, 16, 16), torch.ones(1, 1))},
        time=1,
    )
    with pytest.raises(ValueError, match="generated"):
        model.remember(updated, source="camera plus reflection")
    equal_tree(
        reflected.to_dict(), LatentState.from_dict(reflected.to_dict()).to_dict()
    )
...
def test_task_checkpoint_restores_partial_fulfillment_and_continuation():
    ...
    partial = model.emit(
        state, session, (request("image", USER),), produced_by=AGENT
    ).session
    buffer = io.BytesIO()
...
def test_task_heads_learn_from_text_and_curriculum_splits_are_disjoint():
    training, validation = (
        InstructionEpisodes(count=64),
        InstructionEpisodes(split="validation", count=64),
    )
    assert set(x.request.instruction for x in training.tasks).isdisjoint(
        x.request.instruction for x in validation.tasks
    )
    ...
    b = model.task_predictions(
        state, [TaskSession(replace(task().request, instruction="Ask me what I mean"))]
    )
    assert not torch.equal(a.operation_logits, b.operation_logits)
    assert "task_operation_error" in raw


def test_metadata_changes_reflection_and_each_operation_has_a_bounded_consumer():
    model = build_model(width=16).eval()
    state = model.remember(observed(model), source="camera")
    output = model.emit(state, task(), (request("image"),), produced_by=AGENT).outputs[
        0
    ]
    changed = replace(
        output,
        provenance=replace(
            output.provenance,
            request=replace(output.provenance.request, requested_by=USER),
        ),
    )
    a = model.reflect(state, {"image": output.loopback(state.time)})
    b = model.reflect(state, {"image": changed.loopback(state.time)})
    assert not torch.equal(a.tokens, b.tokens), (
        "Requester attribution must reach neural features"
    )
    for operation in OPERATIONS:
        with torch.no_grad():
            model.task_policy.operation.weight.zero_()
            model.task_policy.operation.bias.fill_(-20)
            model.task_policy.operation.bias[OPERATIONS.index(operation)] = 20
            model.task_policy.modality.weight.zero_()
            model.task_policy.modality.bias.fill_(-20)
            model.task_policy.modality.bias[0] = 20
        result = model.step_task(state, task(), agent=AGENT, max_text_tokens=2)
        assert result.selection.operation == operation
        assert torch.equal(result.state.time, state.time)
        if operation == "imagine":
            assert result.imagined.imagined and result.imagined.time > state.time
        elif operation == "act":
            assert result.action.shape == (1, 2) and (result.action.abs() <= 1).all()
        elif operation == "emit":
            assert len(result.emission.outputs) == 1
        elif operation == "finish":
            assert result.session.finished
        elif operation == "ask":
            assert not result.session.outputs
        else:
            assert not torch.equal(result.state.tokens, state.tokens)


def test_incomplete_tasks_can_abort_and_ancestry_free_snapshots_fail_closed():
    model = build_model(width=16).eval()
    state = observed(model)
    session = task(OutputControl("image", "required", USER))
    failed = session.abort("image decoder unavailable")
    assert failed.remaining == ("image",) and not failed.finished
    restored = TaskSession.from_dict(failed.to_dict())
    step = model.step_task(state, restored, agent=AGENT)
    assert (
        step.selection.operation == "failed" and step.session.failure == failed.failure
    )
    with pytest.raises(ValueError, match="active"):
        model.emit(state, failed, (request("image", USER),), produced_by=AGENT)
    snapshot = state.to_dict()
    snapshot.pop("generated_ancestry")
    with pytest.raises(ValueError, match="ancestry"):
        LatentState.from_dict(snapshot)
    with pytest.raises(ValueError, match="one resolved control"):
        task(
            OutputControl("image", "required", USER),
            OutputControl("image", "automatic", USER),
        )
