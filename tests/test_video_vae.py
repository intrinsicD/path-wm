...
def observation(x, valid=None):
    return Observation(x, torch.arange(x.shape[1], dtype=torch.float64)[None].expand(len(x), -1) / 4, valid)
...
def test_actual_sharing_initial_equivalence_and_unique_checkpoint():
    ...
    assert sum(k.startswith('image.encoder.') for k in saved) == len(image.encoder.state_dict())
    ...
    torch.testing.assert_close(restored(observation(x), sample=False)[0], actual, rtol=0, atol=0)
    disabled = VideoVAE(image, temporal=False)
    assert not list(disabled.temporal.parameters())
    torch.testing.assert_close(disabled(observation(x), sample=False)[0], actual, rtol=0, atol=0)
...
def test_invalid_nan_frames_are_masked_before_encoding_and_have_zero_gradient():
    ...
    x[:, 1] = float('nan'); x.requires_grad_()
    trace = {}
    ...
    clean = x.detach().clone(); clean[:, 1] = 300
    torch.testing.assert_close(y, model(observation(clean, valid), sample=False)[0], rtol=0, atol=0)
...
def test_image_and_video_losses_update_same_encoder_and_keep_resolution_flexible():
    ...
    for h, w in [(9, 11), (12, 8)]:
        ...
        (y.square().mean() + image.square().mean() + 0.001 * p.kl_per_image().mean()).backward()
...
def test_trace_does_not_change_sampling_or_gradients():
    ...
    torch.manual_seed(86); a = model(x)[0]; state_a = torch.get_rng_state()
    torch.manual_seed(86); b = other(x, trace={})[0]; state_b = torch.get_rng_state()
