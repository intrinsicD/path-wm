...
def test_rejected_export_records_coverage_without_running_model(
    tmp_path, monkeypatch, broken_report, offset
):
    ...
    (origin / "run.json").write_text(
        json.dumps(
            dict(identity=dict(settings=settings, data=dict(train=training.identity)))
        )
    )
    source_hash = file_hash(origin / "weights.pt")
    data = MemoryOutputEpisodes(
        64, seed=25073, split="test", curriculum="relocation"
    ).with_scene(background_offset=(offset,) * 3)
    ...
    assert (
        (0 < expected_accepted < len(data)) if offset == 48 else expected_accepted == 0
    )
    monkeypatch.setattr(recipe, "load_model", lambda *args: m)
    monkeypatch.setattr(
        m,
        "observe_history",
        lambda *a, **kw: pytest.fail("Rejected evaluation ran model queries"),
    )
    monkeypatch.setattr(
        torch.optim,
        "AdamW",
        lambda *a, **kw: pytest.fail("Evaluation created optimizer"),
    )
    ...
    if broken_report:
        ...
        with pytest.raises(RuntimeError, match="report failure"):
            recipe.evaluate_export(
                origin / "weights.pt", data, output=out, center_input=True
            )
    else:
        assert (
            recipe.evaluate_export(
    ...
    assert (
        result["completed"]
        and result["gate"] is False
        and result["task_scored"] is False
    )
    ...
    assert status["result"] == "completed" and status["report"] == (
        "failed" if broken_report else "structural-only"
    )
