...
def test_repair_resume_exact(tmp_path):
    ...
    assert all(
        "color_error" in r and "phase_error" in r for r in rows if r["split"] == "train"
    )


def test_cached_draws_match_independent_evaluation():
    from experiments.spatial_vae import color_repair_evaluation, evaluate_images
    from pathwm.models.spatial_vae_v2 import HierarchicalVAE
    from pathwm.io import seed_everything

    seed_everything(9)
    m = HierarchicalVAE(stem_channels=4, channels=[8, 12], latent_channels=2)
    x = torch.rand(3, 3, 32, 36)
    state = torch.get_rng_state().clone()
    calls = []
    hook = m.encoder.register_forward_hook(lambda *args: calls.append(1))
    rows, scores = color_repair_evaluation(m, x, "cpu")
    hook.remove()
    assert len(calls) == 1 and len(scores["sampled_draws"]) == 3
    assert m.training and torch.equal(state, torch.get_rng_state())
    for seed in (58171, 60171):
        independent, _, _ = evaluate_images(m, x, seed=seed, controls=False)
        assert torch.equal(rows[f"sampled_{seed}"], independent["sampled"])
