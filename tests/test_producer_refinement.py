...
def test_refinement_starts_exact_preserves_rng_and_learns_in_two_steps():
    ...
    optimizer = torch.optim.AdamW(d.refinements.parameters(), lr=.001)
...
def test_refined_cached_training_freeze_resume_and_strict_reload(tmp_path):
    ...
    settings.update(steps=4, batch_size=2, wall_seconds=120, readout_stage='native',
                    readout_context='mixed', image_only=True, image_weighting='box',
                    writer_learning=None, standardize_output=False,
                    recall_repair='identity', producer_refinement=True)
    def run(name, resume=False, stop_after=None):
        ...
        train(m, data, data, data, output=tmp_path/name, settings=settings,
    ...
    assert json.loads((tmp_path/'full/run.json').read_text())['identity']['settings']['producer_refinement']
...
def test_refinement_cli_and_settings_mismatch(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(sys,'argv',['recipe','--weights',str(tmp_path/'weights.pt'),
        '--output',str(tmp_path/'fit'),'--repair','identity','--readout-stage','native',
        '--readout-context','mixed','--image-only','--image-weighting','box','--refine-image'])
    original = r.train
    ...
    with pytest.raises(ValueError, match='refinement'):
        original(m,data,data,data,settings=dict(settings,producer_refinement=True),output=tmp_path/'bad')


@pytest.mark.parametrize('flags',[
    ['--evaluate-only','--test-seed','1'],
    ['--repair','identity','--readout-stage','native','--readout-context','mixed'],
    ['--repair','identity','--readout-stage','native','--readout-context','mixed','--image-only','--writer-learning','frozen'],
])
def test_refinement_cli_rejects_unsupported_paths(monkeypatch, flags):
    import sys
    from experiments.memory_output import main
    monkeypatch.setattr(sys,'argv',['recipe','--weights','not-needed.pt','--output','unused','--refine-image',*flags])
