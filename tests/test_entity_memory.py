...
def test_real_matcher_variable_cardinality_and_envelope():
    ...
    assert torch.equal(
        model(inputs)[0], model.match(inputs[:, -1, 0, :8], inputs[:, 0, :, :8])
    )
    assert model.match(torch.randn(3, 8), torch.randn(3, 4, 8)).shape == (3, 5)


def test_atomic_snapshot_and_failed_inference(tmp_path):
    from pathwm.io import atomic_json

    model = Scorer()
    store = EntityMemory(model)
    store.observe("first", torch.eye(8)[0], 0)
    before = store.snapshot()
    path = tmp_path / "memory.json"
    atomic_json(path, before)
    restored = EntityMemory.restore(model, json.loads(path.read_text()))
    assert restored.snapshot() == before
    for timestamp in [-1, float("inf"), 0]:
        with pytest.raises(ValueError):
            restored.observe("invalid", torch.eye(8)[1], timestamp)
    assert restored.snapshot() == before
    malformed = store.snapshot()
    malformed["records"][0]["id"] = 7
    with pytest.raises(ValueError):
        EntityMemory.restore(model, malformed)

    class Broken(Scorer):
        def match(self, query, memory):
            raise RuntimeError("failed inference")

    broken = EntityMemory(Broken())
    broken.observe("first", torch.eye(8)[0], 0)
    before = broken.snapshot()
    with pytest.raises(RuntimeError):
        broken.observe("next", torch.eye(8)[1], 1)
