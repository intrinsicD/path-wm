...
def config():
    ...
    return dict(settings(), dataset="facts", width=16, train_windows=96,
                validation_windows=32, improve_every=0, batch_size=4,
                evaluate_every=1, max_seconds=450.0)
...
def test_semantic_pair_split_and_forward_boundary():
    ...
    assert {l for _, l in a} == set(range(4))
    assert all((e + l) % 4 == 0 for e, l in b)
    ...
    loss = torch.nn.functional.cross_entropy(entity, batch["entities"]) + torch.nn.functional.cross_entropy(location, batch["locations"])
    loss.backward()
...
def test_binding_selector_numeric_and_query_location_order_controls():
    ...
    torch.testing.assert_close(first, bind_facts(entity.flip(1), location.flip(1), torch.tensor([3])))
    assert bind_facts(entity, location.flip(1), torch.tensor([3])).argmax(-1).item() == 2
...
def test_fact_resume_cache_and_no_heldout_selection(tmp_path, monkeypatch):
    ...
    states = [torch.load(tmp_path / n / "last.pt", weights_only=True) for n in ("resumed", "full")]
    for s in states: s["model"].pop("diagnostic_elapsed_seconds")
    for key in ("model", "optimizer", "sampler", "torch", "numpy", "random", "rows", "step"):
