...
def test_pair_center_loss_numeric_reference_and_gradient():
    ...
    loss,parts=direction_loss(logits,labels,pair_center_weight=.1)
    # Class scores4,2: common offset3, SmoothL1(3)=2.5.
    ce=F.cross_entropy(logits,labels)
    torch.testing.assert_close(loss,ce+.25)
    assert float(parts['pair_center_penalty'])==2.5
    penalty_gradient=torch.autograd.grad(loss-ce,logits)[0]
    torch.testing.assert_close(penalty_gradient,torch.tensor([[.05,-.05],[.05,-.05]]))


def test_zero_weight_exact_ce_and_pair_invariances():
    from experiments.video_order import direction_loss

    x=torch.tensor([[2.,-1.],[0.,3.],[-2.,1.],[4.,-3.]],requires_grad=True)
    y=torch.tensor([0,1,1,0])
    loss,_=direction_loss(x,y,pair_center_weight=0)
    reference=F.cross_entropy(x,y)
    assert torch.equal(loss,reference)
    assert torch.equal(torch.autograd.grad(loss,x,retain_graph=True)[0],torch.autograd.grad(reference,x)[0])
    base,_=direction_loss(x,y,pair_center_weight=.1)
    for a,b in [(x+torch.tensor([[8.],[4.],[2.],[16.]]),y),(x.flip(-1),1-y),(x.reshape(2,2,2).flip(1).flatten(0,1),y.reshape(2,2).flip(1).flatten())]:
        other,_=direction_loss(a,b,pair_center_weight=.1)
        torch.testing.assert_close(base,other)
    for bad in (-.1,float('nan'),float('inf')):
        with pytest.raises(ValueError):direction_loss(x,y,pair_center_weight=bad)
    with pytest.raises(ValueError):direction_loss(x,torch.tensor([0,0,1,0]),pair_center_weight=.1)
    with pytest.raises(ValueError):direction_loss(x[:3],y[:3],pair_center_weight=.1)


def test_pair_scores_detect_common_preference_without_counting_it_as_accuracy():
    from experiments.video_order import metrics

    logits=torch.tensor([[[4.,0.],[2.,0.]],[[2.,0.],[0.,2.]]])
    labels=torch.tensor([[0,1],[0,1]])
    m=metrics(logits,labels)
    assert m['accuracy']==.75 and m['pair_accuracy']==.5
    assert m['pair_ordering_accuracy']==1
    assert m['pair_offset_mean_abs']==1.5
    assert m['pair_signal_mean_abs']==1.5
    assert m['pair_bias_fraction']==.375
